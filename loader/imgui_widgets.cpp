...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.size_offset = ImLerp(it_anim->second.size_offset, held ? size_arg / 20 : hovered ? size_arg / 40 : ImVec2(0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, held || hovered ? ImColor(1.f, 1.f, 1.f, 0.25f) : ImColor(0.f, 0.f, 0.f, 0.f), g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(bb.Min + it_anim->second.size_offset, bb.Max - it_anim->second.size_offset, GetColorU32(it_anim->second.rect_color), 4.f);
    window->DrawList->AddRect(bb.Min + it_anim->second.size_offset, bb.Max - it_anim->second.size_offset, second_color, 4.f);

    window->DrawList->AddText(bb.Min + (bb.Max - bb.Min) / 2 - CalcTextSize(label) / 2, ImColor(0.9f, 0.9f, 0.9f, 1.f), label);
...
{
    rtx_spoof_func;
    return window->GetID(axis == ImGuiAxis_X ? OBFUSCATE_STR("#SCROLLX") : OBFUSCATE_STR("#SCROLLY"));
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    const ImGuiID id = window->GetID(OBFUSCATE_STR("#image"));
    PopID();
...
bool ImGui::Product(const char* label, int* v, int number, const char* last_update)
{
    rtx_spoof_func;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    static std::map<ImGuiID, product_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, product_state() });
        it_anim = anim.find(id);
    }

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(300, 35));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v == number ? second_color : background_color, g.IO.DeltaTime * 6.f);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v == number ? main_color : ImColor(0.5f, 0.5f, 0.5f, 1.f), g.IO.DeltaTime * 6.f);

    window->DrawList->AddRect(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.rect_color), 4.f, 0, 2.f);

    window->DrawList->AddText(total_bb.Min + ImVec2(10, (total_bb.Max.y - total_bb.Min.y) / 2 - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.text_color), label);

    PushFont(small_font);
    window->DrawList->AddText(total_bb.Max - ImVec2(10 + CalcTextSize(last_update).x, (total_bb.Max.y - total_bb.Min.y) / 2 + CalcTextSize(last_update).y / 2), GetColorU32(it_anim->second.text_color), last_update);
    PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.circle_radius = ImLerp(it_anim->second.circle_radius, *v ? 14.f : 0.f, g.IO.DeltaTime * 6.f);

    window->DrawList->AddRect(total_bb.Min + ImVec2(275, 10), total_bb.Min + ImVec2(290, 25), main_color, 1.f);

    it_anim->second.checkmark_size = ImLerp(it_anim->second.checkmark_size, *v ? 8.f : 0.f, g.IO.DeltaTime * 6.f);

    it_anim->second.circle_alpha = ImLerp(it_anim->second.circle_alpha, it_anim->second.circle_radius > 10 ? 0.f : 1.f, g.IO.DeltaTime * 6.f);

    PushClipRect(total_bb.Min + ImVec2(276, 11), total_bb.Min + ImVec2(289, 24), true);
    window->DrawList->AddCircleFilled(total_bb.Min + ImVec2(282.5f, 17.5f), it_anim->second.circle_radius, main_color);
    PopClipRect();

    RenderCheckMark(window->DrawList, total_bb.Min + ImVec2(282.5f, 17.5f) - ImVec2(it_anim->second.checkmark_size / 2, it_anim->second.checkmark_size / 2), ImColor(0.1f, 0.1f, 0.1f, 1.f), it_anim->second.checkmark_size);

    window->DrawList->AddText(total_bb.Min + ImVec2(10, (total_bb.Max.y - total_bb.Min.y) / 2 - CalcTextSize(label).y / 2), ImColor(0.3f, 0.3f, 0.3f, 1.f), label);
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (g.LogEnabled)
        LogRenderedText(&label_pos, active ? OBFUSCATE_STR("(x)") : OBFUSCATE_STR("( )"));
    if (label_size.x > 0.0f)
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!overlay)
    {
        ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), OBFUSCATE_STR("%.0f%%"), fraction * 100 + 0.01f);
        overlay = overlay_buf;
    }
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        if (item_visible)
        {
            ...
            if (g.LogEnabled)
                LogRenderedText(&bb.Min, OBFUSCATE_STR("--------------------------------\n"));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held);
    const ImGuiID popup_id = ImHashStr(OBFUSCATE_STR("##ComboPopup"), 0, id);
    ...
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration(OBFUSCATE_STR("{"), OBFUSCATE_STR("}"));
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), OBFUSCATE_STR("##Combo_%02d"), g.BeginPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (!items_getter(data, i, &item_text))
            item_text = OBFUSCATE_STR("*Unknown item*");
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && OBFUSCATE_STR("Invalid ImGuiSliderFlags flags! Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead."));
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        value_changed |= DragScalar(OBFUSCATE_STR(""), data_type, p_data, v_speed, p_min, p_max, format, flags);
        PopID();
        PopItemWidth();
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    bool value_changed = DragScalar(OBFUSCATE_STR("##min"), ImGuiDataType_Float, v_current_min, v_speed, &min_min, &min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    value_changed |= DragScalar(OBFUSCATE_STR("##max"), ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    ImGuiSliderFlags min_flags = flags | ((min_min == min_max) ? ImGuiSliderFlags_ReadOnly : 0);
    bool value_changed = DragInt(OBFUSCATE_STR("##min"), v_current_min, v_speed, min_min, min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    ImGuiSliderFlags max_flags = flags | ((max_min == max_max) ? ImGuiSliderFlags_ReadOnly : 0);
    value_changed |= DragInt(OBFUSCATE_STR("##max"), v_current_max, v_speed, max_min, max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    rtx_spoof_func;
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && OBFUSCATE_STR("Invalid ImGuiSliderFlags flag!  Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead."));
...
bool ImGui::SliderScalarN(const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= SliderScalar(OBFUSCATE_STR(""), data_type, v, v_min, v_max, format, flags);
        PopID();
        PopItemWidth();
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText(OBFUSCATE_STR(""), buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside point of view
        ...
        if (ButtonEx(OBFUSCATE_STR("-"), ImVec2(button_size, button_size), button_flags))
        {
...
bool ImGui::InputScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= InputScalar(OBFUSCATE_STR(""), data_type, p_data, p_step, p_step_fast, format, flags);
        PopID();
        PopItemWidth();
        p_data = (void*)((char*)p_data + type_size);
    }
...
{
    rtx_spoof_func;
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? OBFUSCATE_STR("%08X") : OBFUSCATE_STR("%d");
...
{
    rtx_spoof_func;
    return InputScalarN(label, ImGuiDataType_S32, v, 2, NULL, NULL, OBFUSCATE_STR("%d"), flags);
}

bool ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags)
{
    rtx_spoof_func;
    return InputScalarN(label, ImGuiDataType_S32, v, 3, NULL, NULL, OBFUSCATE_STR("%d"), flags);
}

bool ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags)
{
    rtx_spoof_func;
    return InputScalarN(label, ImGuiDataType_S32, v, 4, NULL, NULL, OBFUSCATE_STR("%d"), flags);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 total_size = ImVec2(300, 40);
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, render_cursor ? main_color : second_color, g.IO.DeltaTime * 6.f);
    it_anim->second.shadow_thinkess = ImLerp(it_anim->second.shadow_thinkess, render_cursor ? 25.f : 5.f, g.IO.DeltaTime * 6.f);

    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddShadowRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.rect_color), it_anim->second.shadow_thinkess, ImVec2(0, 0), 0, 4.f);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, background_color, 4.f, 0);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.rect_color), 4.f, 0, 1.f);
    }


    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos + ImVec2(10, 0) : frame_bb.Min + style.FramePadding + ImVec2(10, 0);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize), rect_pos + ImVec2(rect_size.x, size_arg.y));
        ...
        {
            PushClipRect(frame_bb.Min, ImVec2(it_anim->second.slow_clip, frame_bb.Max.y), true);
            draw_window->DrawList->AddText(frame_bb.Min + ImVec2(15.f, (frame_bb.Max.y - frame_bb.Min.y) / 2 - CalcTextSize(buf_display).y / 2), ImColor(0.6f, 0.6f, 0.6f, 1.f), buf_display);
            PopClipRect();
        }
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 7.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y);

            it_anim->second.slow_clip = ImLerp(it_anim->second.slow_clip, cursor_screen_rect.Min.x, g.IO.DeltaTime * 10.f);
            it_anim->second.alpha_line = ImLerp(it_anim->second.alpha_line, cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect) ? 1.f : 0.f, g.IO.DeltaTime * 8.f);
            it_anim->second.slow_moveX0 = ImLerp(it_anim->second.slow_moveX0, cursor_screen_rect.Min, g.IO.DeltaTime * 25.f);
            it_anim->second.slow_moveX1 = ImLerp(it_anim->second.slow_moveX1, cursor_screen_rect.GetBL(), g.IO.DeltaTime * 25.f);

            draw_window->DrawList->AddLine(it_anim->second.slow_moveX0, it_anim->second.slow_moveX0 + ImVec2(0, g.FontSize / 1.5f), ImColor(1.f, 1.f, 1.f, it_anim->second.alpha_line));
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(frame_bb.Min + ImVec2(15.f, (frame_bb.Max.y - frame_bb.Min.y) / 2 - CalcTextSize(buf_display).y / 2), ImColor(0.3f, 0.3f, 0.3f, 1.f), buf_display);
        }
    }
    ...
    if (!buf[0])
        window->DrawList->AddText(frame_bb.Min + ImVec2(15.f, (frame_bb.Max.y - frame_bb.Min.y) / 2 - CalcTextSize(buf_display).y / 2), ImColor(0.5f, 0.5f, 0.5f, 1.f), label);
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    Text(OBFUSCATE_STR("ID: 0x%08X, ActiveID: 0x%08X"), state->ID, g.ActiveId);
    DebugLocateItemOnHover(state->ID);
    Text(OBFUSCATE_STR("CurLenW: %d, CurLenA: %d, Cursor: %d, Selection: %d..%d"), state->CurLenW, state->CurLenA, stb_state->cursor, stb_state->select_start, stb_state->select_end);
    Text(OBFUSCATE_STR("has_preferred_x: %d (%.2f)"), stb_state->has_preferred_x, stb_state->preferred_x);
    Text(OBFUSCATE_STR("undo_point: %d, redo_point: %d, undo_char_point: %d, redo_char_point: %d"), undo_state->undo_point, undo_state->redo_point, undo_state->undo_char_point, undo_state->redo_char_point);
    if (BeginChild(OBFUSCATE_STR("undopoints"), ImVec2(0.0f, GetTextLineHeight() * 15), true)) // Visualize undo state
    {
        ...
        for (int n = 0; n < STB_TEXTEDIT_UNDOSTATECOUNT; n++)
        {
            ...
            Text(OBFUSCATE_STR("%c [%02d] where %03d, insert %03d, delete %03d, char_storage %03d \"%s\""),
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            if (!(flags & ImGuiColorEditFlags_NoOptions))
                OpenPopupOnItemClick(OBFUSCATE_STR("context"), ImGuiPopupFlags_MouseButtonRight);
        }
    }
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        char buf[64];
        if (alpha)
            ImFormatString(buf, IM_ARRAYSIZE(buf), OBFUSCATE_STR("#%02X%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
        else
            ImFormatString(buf, IM_ARRAYSIZE(buf), OBFUSCATE_STR("#%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
        SetNextItemWidth(w_inputs);
        if (InputText(OBFUSCATE_STR("##Text"), buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* p = buf;
            ...
            int r;
            if (alpha)
                r = sscanf(p, OBFUSCATE_STR("%02X%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2], (unsigned int*)&i[3]); // Treat at unsigned (%X is unsigned)
            else
                r = sscanf(p, OBFUSCATE_STR("%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2]);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton(OBFUSCATE_STR("##ColorButton"), col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                // Store current color and open a picker
                g.ColorPickerRef = col_v4;
                OpenPopup(OBFUSCATE_STR("picker"));
        ...
        if (BeginPopup(OBFUSCATE_STR("picker")))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                value_changed |= ColorPicker4(OBFUSCATE_STR("##picker"), col, picker_flags, &g.ColorPickerRef.x);
            }
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        InvisibleButton(OBFUSCATE_STR("hsv"), ImVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    {
        // SV rectangle logic
        InvisibleButton(OBFUSCATE_STR("sv"), ImVec2(sv_picker_size, sv_picker_size));
        if (IsItemActive())
        {
        ...
        InvisibleButton(OBFUSCATE_STR("hue"), ImVec2(bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    if (alpha_bar)
    {
        ...
        InvisibleButton(OBFUSCATE_STR("alpha"), ImVec2(bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        if ((flags & ImGuiColorEditFlags_NoLabel))
            Text(OBFUSCATE_STR("Current"));

        ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip;
        ColorButton(OBFUSCATE_STR("##current"), col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text(OBFUSCATE_STR("Original"));
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton(OBFUSCATE_STR("##original"), ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
            {
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4(OBFUSCATE_STR("##rgb"), col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(OBFUSCATE_STR("##hsv"), col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(OBFUSCATE_STR("##hex"), col, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton(OBFUSCATE_STR("##preview"), cf, (flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
    SameLine();
    if ((flags & ImGuiColorEditFlags_InputRGB) || !(flags & ImGuiColorEditFlags_InputMask_))
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text(OBFUSCATE_STR("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)"), cr, cg, cb, cr, cg, cb, col[0], col[1], col[2]);
        else
            Text(OBFUSCATE_STR("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)"), cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1], col[2], col[3]);
    }
    else if (flags & ImGuiColorEditFlags_InputHSV)
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text(OBFUSCATE_STR("H: %.3f, S: %.3f, V: %.3f"), col[0], col[1], col[2]);
        else
            Text(OBFUSCATE_STR("H: %.3f, S: %.3f, V: %.3f, A: %.3f"), col[0], col[1], col[2], col[3]);
    }
    EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_inputs && !allow_opt_datatype) || !BeginPopup(OBFUSCATE_STR("context")))
        return;
    ImGuiContext& g = *GImGui;
    ImGuiColorEditFlags opts = g.ColorEditOptions;
    if (allow_opt_inputs)
    {
        if (RadioButton(OBFUSCATE_STR("RGB"), (opts & ImGuiColorEditFlags_DisplayRGB) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayRGB;
        if (RadioButton(OBFUSCATE_STR("HSV"), (opts & ImGuiColorEditFlags_DisplayHSV) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHSV;
        if (RadioButton(OBFUSCATE_STR("Hex"), (opts & ImGuiColorEditFlags_DisplayHex) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHex;
    }
    if (allow_opt_datatype)
    {
        if (allow_opt_inputs) Separator();
        if (RadioButton(OBFUSCATE_STR("0..255"), (opts & ImGuiColorEditFlags_Uint8) != 0)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Uint8;
        if (RadioButton(OBFUSCATE_STR("0.00..1.00"), (opts & ImGuiColorEditFlags_Float) != 0)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Float;
    }

    if (allow_opt_inputs || allow_opt_datatype)
        Separator();
    if (Button(OBFUSCATE_STR("Copy as.."), ImVec2(-1, 0)))
        OpenPopup(OBFUSCATE_STR("Copy"));
    if (BeginPopup(OBFUSCATE_STR("Copy")))
    {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
        ImFormatString(buf, IM_ARRAYSIZE(buf), OBFUSCATE_STR("(%.3ff, %.3ff, %.3ff, %.3ff)"), col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
        if (Selectable(buf))
            SetClipboardText(buf);
        ImFormatString(buf, IM_ARRAYSIZE(buf), OBFUSCATE_STR("(%d,%d,%d,%d)"), cr, cg, cb, ca);
        if (Selectable(buf))
            SetClipboardText(buf);
        ImFormatString(buf, IM_ARRAYSIZE(buf), OBFUSCATE_STR("#%02X%02X%02X"), cr, cg, cb);
        if (Selectable(buf))
            SetClipboardText(buf);
        if (!(flags & ImGuiColorEditFlags_NoAlpha))
        {
            ImFormatString(buf, IM_ARRAYSIZE(buf), OBFUSCATE_STR("#%02X%02X%02X%02X"), cr, cg, cb, ca);
            if (Selectable(buf))
                SetClipboardText(buf);
        }
        EndPopup();
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_picker && !allow_opt_alpha_bar) || !BeginPopup(OBFUSCATE_STR("context")))
        return;
    ImGuiContext& g = *GImGui;
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable(OBFUSCATE_STR("##selectable"), false, 0, picker_size)) // By default, Selectable() is closing popup
                g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags_PickerMask_) | (picker_flags & ImGuiColorEditFlags_PickerMask_);
            SetCursorScreenPos(backup_pos);
            ImVec4 previewing_ref_col;
            memcpy(&previewing_ref_col, ref_col, sizeof(float) * ((picker_flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4));
            ColorPicker4(OBFUSCATE_STR("##previewing_picker"), &previewing_ref_col.x, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
    if (allow_opt_alpha_bar)
    {
        ...
        CheckboxFlags(OBFUSCATE_STR("Alpha Bar"), &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    EndPopup();
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    else
    {
        ...
        if (g.LogEnabled)
            LogSetNextTextDecoration(OBFUSCATE_STR(">"), NULL);
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        ImGuiID close_button_id = GetIDWithSeed(OBFUSCATE_STR("#CLOSE"), NULL, id);
...
void ImGui::EndListBox()
{
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_ASSERT((window->Flags & ImGuiWindowFlags_ChildWindow) && OBFUSCATE_STR("Mismatched BeginListBox/EndListBox calls. Did you test the return value of BeginListBox?"));
    IM_UNUSED(window);
    ...
    if (values_count >= values_count_min)
    {
        ...
        if (hovered && inner_bb.Contains(g.IO.MousePos))
        {
            ...
            if (plot_type == ImGuiPlotType_Lines)
                SetTooltip(OBFUSCATE_STR("%d: %8.4g\n%d: %8.4g"), v_idx, v0, v_idx + 1, v1);
            else if (plot_type == ImGuiPlotType_Histogram)
                SetTooltip(OBFUSCATE_STR("%d: %8.4g"), v_idx, v0);
            idx_hovered = v_idx;
        }
...
{
    rtx_spoof_func;
    Text(OBFUSCATE_STR("%s: %s"), prefix, (b ? OBFUSCATE_STR("true") : OBFUSCATE_STR("false")));
}

void ImGui::Value(const char* prefix, int v)
{
    rtx_spoof_func;
    Text(OBFUSCATE_STR("%s: %d"), prefix, v);
}
...
void ImGui::Value(const char* prefix, float v, const char* float_format)
{
    ...
    {
        char fmt[64];
        ImFormatString(fmt, IM_ARRAYSIZE(fmt), OBFUSCATE_STR("%%s: %s"), float_format);
        Text(fmt, prefix, v);
    }
    else
    {
        Text(OBFUSCATE_STR("%s: %.3f"), prefix, v);
    }
}
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = BeginViewportSideBar(OBFUSCATE_STR("##MainMenuBar"), viewport, ImGuiDir_Up, height, window_flags);
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        pressed = Selectable(OBFUSCATE_STR(""), menu_is_open, selectable_flags, ImVec2(w, 0.0f));
    ...
    else
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        pressed = Selectable(OBFUSCATE_STR(""), menu_is_open, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        RenderText(text_pos, label);
        if (icon_w > 0.0f)
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        pressed = Selectable(OBFUSCATE_STR(""), selected, selectable_flags, ImVec2(w, 0.0f));
        PopStyleVar();
    ...
    else
    {
        ...
        pressed = Selectable(OBFUSCATE_STR(""), false, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
...
void ImGui::EndTabBar()
{
    ...
    if (tab_bar == NULL)
    {
        IM_ASSERT_USER_ERROR(tab_bar != NULL, OBFUSCATE_STR("Mismatched BeginTabBar()/EndTabBar()!"));
        return;
    }
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    if (ArrowButtonEx(OBFUSCATE_STR("##<"), ImGuiDir_Left, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = -1;
    window->DC.CursorPos = ImVec2(x + arrow_button_size.x, tab_bar->BarRect.Min.y);
    if (ArrowButtonEx(OBFUSCATE_STR("##>"), ImGuiDir_Right, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = +1;
    PopStyleColor(2);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo(OBFUSCATE_STR("##v"), NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
...
bool ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (tab_bar == NULL)
    {
        IM_ASSERT_USER_ERROR(tab_bar, OBFUSCATE_STR("Needs to be called between BeginTabBar() and EndTabBar()!"));
        return false;
    }
...
void ImGui::EndTabItem()
{
    ...
    if (tab_bar == NULL)
    {
        IM_ASSERT_USER_ERROR(tab_bar != NULL, OBFUSCATE_STR("Needs to be called between BeginTabBar() and EndTabBar()!"));
        return;
    }
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    const ImGuiID close_button_id = p_open ? GetIDWithSeed(OBFUSCATE_STR("#CLOSE"), NULL, id) : 0;
    bool just_closed;
    bool text_clipped;
    ...
    if (text_clipped && g.HoveredId == id && !held)
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(tab->Flags & ImGuiTabItemFlags_NoTooltip))
            if (IsItemHovered(ImGuiHoveredFlags_DelayNormal))
                SetTooltip(OBFUSCATE_STR("%.*s"), (int)(FindRenderedTextEnd(label) - label), label);

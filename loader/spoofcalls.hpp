...
#define rtx_spoof_func CallSpoofer::SpoofFunction spoof(_AddressOfReturnAddress());
#ifdef _KERNEL_MODE
#define rtx_spoof_call(ret_type,name) (CallSpoofer::SafeCall<ret_type, std::remove_reference_t<decltype(*name)>>(name))
#else
#define rtx_spoof_call(name) (CallSpoofer::SafeCall(name))
#endif
...
namespace CallSpoofer
{
	...
	public:
		uintptr_t temp = 0;
		const uintptr_t xor_key = 0x0ff0f0ff0f0ff00f;
	...
	public:
		...
		__forceinline decltype(auto) operator()(Args&&... args)
		{
			...
			if (!p_shellcode)
			{
				...
				orig_generator[count] = (p_shell_code_generator_type)self_addr;
